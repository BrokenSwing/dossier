import { type CollectionId, ComputeAuth, type DocumentId, InternalError, NotFoundError } from "@dossier/shared";
...
export const makeFakeStorageClientLayer = (initialDocs: FakeDocument[]) => {
  ...
  const layer = Layer.sync(StorageClient, () => ({
    ...
    CreateDocumentMeta: (payload: { name: string; format: "pdf" | "jpg" | "png"; tagNames: string[]; collectionIds: CollectionId[] }, _opts?: unknown) => {
