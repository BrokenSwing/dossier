  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return BlobStore.of({
        write: (key, data) =>
          fs
            .writeFile(pathSvc.join(dir, key), data)
            .pipe(Effect.mapError((e) => new InternalError({ message: `Failed to write blob ${key}: ${e.message}` }))),

        readStream: (key) =>
          fs.stream(pathSvc.join(dir, key)).pipe(Stream.mapError((e) => new InternalError({ message: `Failed to read blob ${key}: ${e.message}` }))),

        delete: (key) => fs.remove(pathSvc.join(dir, key)).pipe(Effect.catchAll(() => Effect.void)),
      });
    }),
  );
