      ...
      Effect.gen(function*() {
        ...
        if (Option.isNone(raw)) {
          return yield* Effect.fail(
            new InvalidSessionError({ message: "Missing session token" })
          )
        }
        ...
        const payload = yield* Effect.try({
          try: () =>
            ...
            new InvalidSessionError({ message: "Invalid or expired token" }),
        })
        ...
        const sessions = yield* sql<{ id: string }>`
          ...
          LIMIT 1`.pipe(
          Effect.mapError(
            () => new InvalidSessionError({ message: "Internal session lookup failed" })
          )
        )

        if (sessions.length === 0) {
          return yield* Effect.fail(
            new InvalidSessionError({ message: "Session not found or revoked" })
          )
        }
