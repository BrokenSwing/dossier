...
export const findById = (id: DocumentId, userId: UserId) =>
  Effect.gen(function*() {
    ...
    const [tags, colls] = yield* Effect.all([
      fetchDocumentTags(id),
      fetchDocumentCollections(id),
    ])
    return Option.some(rowToMeta(row, tags.map((t) => t.name), colls.map((c) => c.collection_id)))
  })
  ...
  Effect.gen(function*() {
    ...
    return yield* sql.withTransaction(
      Effect.gen(function*() {
        ...
        yield* sql`DELETE FROM document_tags WHERE document_id = ${documentId}`
          .pipe(Effect.mapError(mapSqlError("updateDocumentTags delete")))
...
export const updateDocumentCollections = (
  ...
  Effect.gen(function*() {
    ...
    yield* sql.withTransaction(
      Effect.gen(function*() {
        yield* sql`DELETE FROM collection_documents WHERE document_id = ${documentId}`
          .pipe(Effect.mapError(mapSqlError("updateDocumentCollections delete")))
...
const encodeCursor = (v: string, id: string): string =>
