...
const BlobTestLayer = Layer.mergeAll(
  ...
  Layer.scopedDiscard(
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() => fileSystem.remove(dir, { recursive: true }).pipe(Effect.orElse(() => Effect.void)));
    }),
  ),
  ...
  describe("readStream", () => {
    it.effect("reads back the same bytes that were written", () =>
      Effect.gen(function* () {
        ...
        const chunks = yield* Stream.runCollect(store.readStream(key)).pipe(Effect.map((chunk) => [...chunk]));
